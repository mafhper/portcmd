...
// Import Audits
const validators = require('../packages/validators/index.cjs'); // Using relative internal path as we are inside the package
...
const AVAILABLE_AUDITS = {
    ...
    // New Validators (via barrel)
    'contrast': validators.contrast,
    'i18n': validators.i18n,
...
async function main() {
    ...
    // --- CANONICAL JSON TRANSFORMATION (Quality Reports 2.0) ---
    ...
    const canonicalResult = {
        ...
        metrics: {
            ...
            ...result.raw // Moving raw data into metrics/
        },
        violations: result.violations.map(v => ({
            area: v.area || 'unknown',
            metric: v.metric || 'unknown',
            value: v.value,
    ...
    // Save the CANONICAL result, not the raw runner result
    const jsonPath = JsonReporter.save(canonicalResult, reportDir, `${filename}.json`);
    console.log(`\n📄 JSON Report: ${jsonPath}`);

    JsonReporter.save(canonicalResult, reportDir, 'quality-latest.json');

    const MarkdownReporter = require('../packages/reporters/markdown.cjs');
    // Markdown generation might need adaptation if it relies on old structure, 
    // but usually it uses the same unified object. Passing canonicalResult might break it if it expects root.raw
    // For now, let's try passing canonicalResult, but mapped back if needed or update MarkdownReporter later.
    // To be safe, let's keep passing 'result' to MarkdownReporter for now as it hasn't been refactored yet.
